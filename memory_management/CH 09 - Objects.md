...
* If "a" is a vector3:
  ...
  2. Otherwise:
    ...
    3. Return the vector
* If "a" is a array:
  1. If "b" is not a `ARRAY`, invalid operation, return `NULL`
  2. Otherwise:
    1. Create a `new_snek_array` with the combined length of two arrays
    2. Iterate over each index in "a" and use `snek_array_set` and `snek_array_get` to copy values for "a" to the new array
    3. Repeat for "b"
    4. Return the array
```C
// End of lesson .c file
