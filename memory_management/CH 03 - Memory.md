...
Since the beginning of this course we've been doing these shenanigans to be able to print strings:

```C
char *msg = "ssh terminal.shop for the best coffee";
```

You were told not to worry about the weird `char *` syntax. Now that we understand a bit about pointers, let's dive into it. In the example above, `msg` is a pointer to the first character of the string `"ssh terminal.shop for the best coffee"` . Which is a [C string](https://en.wikipedia.org/wiki/C_string_handling). C strings are:
  * How we repersent text in C programs
...
printf("%s %s\n", str1, str2);
```
The output is same. Let's break down the memory of this example:
```C
...
_NOTE! There is a bunch of garbage at the end of the string._
...
The `sttrcat` function appends the string `"lang!"` to the end of the string `"Snek"`, but smartly uses the null terminator to know where to start appending. It doesn't know the length of the string, but it knows where it ends.
